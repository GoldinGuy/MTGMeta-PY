...
def most_common_cards(_deck, k):
    _deck.sort(key=lambda _deck: _deck[0], reverse=True)
    return [card[1] for card in _deck[:k]]
    ...
    for deck in decks_json:
        ...
        for card in deck['main']:
            ...
            if card_name not in IGNORE:
                cards_w_ig.append(card_name)
            if any(card_name in d for d in unique_cards):
                unique_cards[next((i for i, d in enumerate(unique_cards) if card_name in d), None)][
                    card_name] += quantity
            elif card_name not in IGNORE:
                unique_cards.append({card_name: quantity})
        decks.append(cards_in_deck)
    format_json['total_cards_parsed'] = len(all_cards)
    all_cards = list(set(all_cards))
    format_json['unique_cards_parsed'] = len(all_cards)


    def deck_to_vector(input_deck):
        v = [0] * len(all_cards)
        for x, name in enumerate(all_cards):
    ...
    for CLUSTER_ID in range(NUM_CLUSTERS):
        ...
        for deck in decks_json:
            ...
            if num_similar_cards > max_similar_cards:
                ...
                best_fit_deck['deck'] = deck['main']
                best_fit_deck['sb'] = deck['sb']
        # PRINT CLUSTER
